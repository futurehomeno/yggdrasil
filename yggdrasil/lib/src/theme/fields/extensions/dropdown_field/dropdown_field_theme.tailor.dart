// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint, unused_element, unnecessary_cast

part of 'dropdown_field_theme.dart';

// **************************************************************************
// TailorAnnotationsGenerator
// **************************************************************************

class YgDropdownFieldTheme extends ThemeExtension<YgDropdownFieldTheme> {
  const YgDropdownFieldTheme({
    required this.menuAnimationCurve,
    required this.menuAnimationDuration,
    required this.menuElevation,
    required this.menuItemBackground,
    required this.menuItemPadding,
    required this.menuItemPrefixPadding,
    required this.menuItemRadius,
    required this.menuItemSubtitleStyle,
    required this.menuItemSuffixPadding,
    required this.menuItemTitleStyle,
    required this.menuPadding,
    required this.menuRadius,
    required this.menuToFieldPadding,
  });

  final Curve menuAnimationCurve;
  final Duration menuAnimationDuration;
  final double menuElevation;
  final Color menuItemBackground;
  final EdgeInsets menuItemPadding;
  final EdgeInsets menuItemPrefixPadding;
  final BorderRadius menuItemRadius;
  final TextStyle menuItemSubtitleStyle;
  final EdgeInsets menuItemSuffixPadding;
  final TextStyle menuItemTitleStyle;
  final EdgeInsets menuPadding;
  final BorderRadius menuRadius;
  final double menuToFieldPadding;

  static final YgDropdownFieldTheme consumerLight = YgDropdownFieldTheme(
    menuAnimationCurve: _$YgDropdownFieldTheme.menuAnimationCurve[0],
    menuAnimationDuration: _$YgDropdownFieldTheme.menuAnimationDuration[0],
    menuElevation: _$YgDropdownFieldTheme.menuElevation[0],
    menuItemBackground: _$YgDropdownFieldTheme.menuItemBackground[0],
    menuItemPadding: _$YgDropdownFieldTheme.menuItemPadding[0],
    menuItemPrefixPadding: _$YgDropdownFieldTheme.menuItemPrefixPadding[0],
    menuItemRadius: _$YgDropdownFieldTheme.menuItemRadius[0],
    menuItemSubtitleStyle: _$YgDropdownFieldTheme.menuItemSubtitleStyle[0],
    menuItemSuffixPadding: _$YgDropdownFieldTheme.menuItemSuffixPadding[0],
    menuItemTitleStyle: _$YgDropdownFieldTheme.menuItemTitleStyle[0],
    menuPadding: _$YgDropdownFieldTheme.menuPadding[0],
    menuRadius: _$YgDropdownFieldTheme.menuRadius[0],
    menuToFieldPadding: _$YgDropdownFieldTheme.menuToFieldPadding[0],
  );

  static final YgDropdownFieldTheme consumerDark = YgDropdownFieldTheme(
    menuAnimationCurve: _$YgDropdownFieldTheme.menuAnimationCurve[1],
    menuAnimationDuration: _$YgDropdownFieldTheme.menuAnimationDuration[1],
    menuElevation: _$YgDropdownFieldTheme.menuElevation[1],
    menuItemBackground: _$YgDropdownFieldTheme.menuItemBackground[1],
    menuItemPadding: _$YgDropdownFieldTheme.menuItemPadding[1],
    menuItemPrefixPadding: _$YgDropdownFieldTheme.menuItemPrefixPadding[1],
    menuItemRadius: _$YgDropdownFieldTheme.menuItemRadius[1],
    menuItemSubtitleStyle: _$YgDropdownFieldTheme.menuItemSubtitleStyle[1],
    menuItemSuffixPadding: _$YgDropdownFieldTheme.menuItemSuffixPadding[1],
    menuItemTitleStyle: _$YgDropdownFieldTheme.menuItemTitleStyle[1],
    menuPadding: _$YgDropdownFieldTheme.menuPadding[1],
    menuRadius: _$YgDropdownFieldTheme.menuRadius[1],
    menuToFieldPadding: _$YgDropdownFieldTheme.menuToFieldPadding[1],
  );

  static final YgDropdownFieldTheme professionalLight = YgDropdownFieldTheme(
    menuAnimationCurve: _$YgDropdownFieldTheme.menuAnimationCurve[2],
    menuAnimationDuration: _$YgDropdownFieldTheme.menuAnimationDuration[2],
    menuElevation: _$YgDropdownFieldTheme.menuElevation[2],
    menuItemBackground: _$YgDropdownFieldTheme.menuItemBackground[2],
    menuItemPadding: _$YgDropdownFieldTheme.menuItemPadding[2],
    menuItemPrefixPadding: _$YgDropdownFieldTheme.menuItemPrefixPadding[2],
    menuItemRadius: _$YgDropdownFieldTheme.menuItemRadius[2],
    menuItemSubtitleStyle: _$YgDropdownFieldTheme.menuItemSubtitleStyle[2],
    menuItemSuffixPadding: _$YgDropdownFieldTheme.menuItemSuffixPadding[2],
    menuItemTitleStyle: _$YgDropdownFieldTheme.menuItemTitleStyle[2],
    menuPadding: _$YgDropdownFieldTheme.menuPadding[2],
    menuRadius: _$YgDropdownFieldTheme.menuRadius[2],
    menuToFieldPadding: _$YgDropdownFieldTheme.menuToFieldPadding[2],
  );

  static final YgDropdownFieldTheme professionalDark = YgDropdownFieldTheme(
    menuAnimationCurve: _$YgDropdownFieldTheme.menuAnimationCurve[3],
    menuAnimationDuration: _$YgDropdownFieldTheme.menuAnimationDuration[3],
    menuElevation: _$YgDropdownFieldTheme.menuElevation[3],
    menuItemBackground: _$YgDropdownFieldTheme.menuItemBackground[3],
    menuItemPadding: _$YgDropdownFieldTheme.menuItemPadding[3],
    menuItemPrefixPadding: _$YgDropdownFieldTheme.menuItemPrefixPadding[3],
    menuItemRadius: _$YgDropdownFieldTheme.menuItemRadius[3],
    menuItemSubtitleStyle: _$YgDropdownFieldTheme.menuItemSubtitleStyle[3],
    menuItemSuffixPadding: _$YgDropdownFieldTheme.menuItemSuffixPadding[3],
    menuItemTitleStyle: _$YgDropdownFieldTheme.menuItemTitleStyle[3],
    menuPadding: _$YgDropdownFieldTheme.menuPadding[3],
    menuRadius: _$YgDropdownFieldTheme.menuRadius[3],
    menuToFieldPadding: _$YgDropdownFieldTheme.menuToFieldPadding[3],
  );

  static final themes = [
    consumerLight,
    consumerDark,
    professionalLight,
    professionalDark,
  ];

  @override
  YgDropdownFieldTheme copyWith({
    Curve? menuAnimationCurve,
    Duration? menuAnimationDuration,
    double? menuElevation,
    Color? menuItemBackground,
    EdgeInsets? menuItemPadding,
    EdgeInsets? menuItemPrefixPadding,
    BorderRadius? menuItemRadius,
    TextStyle? menuItemSubtitleStyle,
    EdgeInsets? menuItemSuffixPadding,
    TextStyle? menuItemTitleStyle,
    EdgeInsets? menuPadding,
    BorderRadius? menuRadius,
    double? menuToFieldPadding,
  }) {
    return YgDropdownFieldTheme(
      menuAnimationCurve: menuAnimationCurve ?? this.menuAnimationCurve,
      menuAnimationDuration: menuAnimationDuration ?? this.menuAnimationDuration,
      menuElevation: menuElevation ?? this.menuElevation,
      menuItemBackground: menuItemBackground ?? this.menuItemBackground,
      menuItemPadding: menuItemPadding ?? this.menuItemPadding,
      menuItemPrefixPadding: menuItemPrefixPadding ?? this.menuItemPrefixPadding,
      menuItemRadius: menuItemRadius ?? this.menuItemRadius,
      menuItemSubtitleStyle: menuItemSubtitleStyle ?? this.menuItemSubtitleStyle,
      menuItemSuffixPadding: menuItemSuffixPadding ?? this.menuItemSuffixPadding,
      menuItemTitleStyle: menuItemTitleStyle ?? this.menuItemTitleStyle,
      menuPadding: menuPadding ?? this.menuPadding,
      menuRadius: menuRadius ?? this.menuRadius,
      menuToFieldPadding: menuToFieldPadding ?? this.menuToFieldPadding,
    );
  }

  @override
  YgDropdownFieldTheme lerp(covariant ThemeExtension<YgDropdownFieldTheme>? other, double t) {
    if (other is! YgDropdownFieldTheme) return this as YgDropdownFieldTheme;
    return YgDropdownFieldTheme(
      menuAnimationCurve: t < 0.5 ? menuAnimationCurve : other.menuAnimationCurve,
      menuAnimationDuration: t < 0.5 ? menuAnimationDuration : other.menuAnimationDuration,
      menuElevation: t < 0.5 ? menuElevation : other.menuElevation,
      menuItemBackground: Color.lerp(menuItemBackground, other.menuItemBackground, t)!,
      menuItemPadding: t < 0.5 ? menuItemPadding : other.menuItemPadding,
      menuItemPrefixPadding: t < 0.5 ? menuItemPrefixPadding : other.menuItemPrefixPadding,
      menuItemRadius: t < 0.5 ? menuItemRadius : other.menuItemRadius,
      menuItemSubtitleStyle: TextStyle.lerp(menuItemSubtitleStyle, other.menuItemSubtitleStyle, t)!,
      menuItemSuffixPadding: t < 0.5 ? menuItemSuffixPadding : other.menuItemSuffixPadding,
      menuItemTitleStyle: TextStyle.lerp(menuItemTitleStyle, other.menuItemTitleStyle, t)!,
      menuPadding: t < 0.5 ? menuPadding : other.menuPadding,
      menuRadius: t < 0.5 ? menuRadius : other.menuRadius,
      menuToFieldPadding: t < 0.5 ? menuToFieldPadding : other.menuToFieldPadding,
    );
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is YgDropdownFieldTheme &&
            const DeepCollectionEquality().equals(menuAnimationCurve, other.menuAnimationCurve) &&
            const DeepCollectionEquality().equals(menuAnimationDuration, other.menuAnimationDuration) &&
            const DeepCollectionEquality().equals(menuElevation, other.menuElevation) &&
            const DeepCollectionEquality().equals(menuItemBackground, other.menuItemBackground) &&
            const DeepCollectionEquality().equals(menuItemPadding, other.menuItemPadding) &&
            const DeepCollectionEquality().equals(menuItemPrefixPadding, other.menuItemPrefixPadding) &&
            const DeepCollectionEquality().equals(menuItemRadius, other.menuItemRadius) &&
            const DeepCollectionEquality().equals(menuItemSubtitleStyle, other.menuItemSubtitleStyle) &&
            const DeepCollectionEquality().equals(menuItemSuffixPadding, other.menuItemSuffixPadding) &&
            const DeepCollectionEquality().equals(menuItemTitleStyle, other.menuItemTitleStyle) &&
            const DeepCollectionEquality().equals(menuPadding, other.menuPadding) &&
            const DeepCollectionEquality().equals(menuRadius, other.menuRadius) &&
            const DeepCollectionEquality().equals(menuToFieldPadding, other.menuToFieldPadding));
  }

  @override
  int get hashCode {
    return Object.hash(
      runtimeType.hashCode,
      const DeepCollectionEquality().hash(menuAnimationCurve),
      const DeepCollectionEquality().hash(menuAnimationDuration),
      const DeepCollectionEquality().hash(menuElevation),
      const DeepCollectionEquality().hash(menuItemBackground),
      const DeepCollectionEquality().hash(menuItemPadding),
      const DeepCollectionEquality().hash(menuItemPrefixPadding),
      const DeepCollectionEquality().hash(menuItemRadius),
      const DeepCollectionEquality().hash(menuItemSubtitleStyle),
      const DeepCollectionEquality().hash(menuItemSuffixPadding),
      const DeepCollectionEquality().hash(menuItemTitleStyle),
      const DeepCollectionEquality().hash(menuPadding),
      const DeepCollectionEquality().hash(menuRadius),
      const DeepCollectionEquality().hash(menuToFieldPadding),
    );
  }
}
